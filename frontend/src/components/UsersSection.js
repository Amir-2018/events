  ...
  return (
    <div className="w-full">
      <div className="flex justify-between items-center mb-6">
        <div>
          <h1 className="text-xl font-bold text-gray-900 mb-1">Gestion de l'équipe</h1>
          <p className="text-xs text-gray-500">Gérez les administrateurs et super-administrateurs du système</p>
        </div>
        <button
          onClick={handleCreate}
          className="bg-[#31a7df] text-white px-4 py-2 rounded-lg font-medium text-sm hover:bg-[#2596d1] transition-all flex items-center gap-2"
        >
          ...
          <table className="w-full">
            <thead>
              <tr className="bg-gray-50 border-b border-gray-100">
                <th className="text-left px-6 py-4 font-bold text-gray-700 text-xs tracking-wider">Membre</th>
                <th className="text-left py-4 font-bold text-gray-700 text-xs tracking-wider">Identifiant</th>
                <th className="text-left py-4 font-bold text-gray-700 text-xs tracking-wider">Rôle</th>
                <th className="text-left py-4 font-bold text-gray-700 text-xs tracking-wider">Statut</th>
                <th className="text-left py-4 font-bold text-gray-700 text-xs tracking-wider">Créé le</th>
                <th className="text-right px-6 py-4 font-bold text-gray-700 text-xs tracking-wider">Actions</th>
              </tr>
            </thead>
                ...
                <tr key={u.id} className="hover:bg-blue-50/30 transition-colors">
                  <td className="px-6 py-4">
                    <div className="flex items-center">
                      <div className="w-8 h-8 rounded-lg bg-blue-50 flex items-center justify-center text-[#2596d1] font-bold mr-3 text-xs">
                        {u.prenom?.[0]}{u.nom?.[0]}
                      </div>
                      <span className="font-bold text-gray-900 text-sm">{u.prenom} {u.nom}</span>
                    </div>
                  </td>
                  <td className="py-4 font-medium text-gray-600 text-sm">{u.username}</td>
                  <td className="py-4">
                  ...
                  <td className="py-4 text-gray-500 font-medium text-sm">
                  ...
                  <td className="px-6 py-4 text-right flex justify-end gap-1">
                    {u.status === 'pending' && (
                      <div className="flex gap-1 mr-2 bg-gray-50 p-1 rounded-lg border border-gray-100">
                        <button 
                          ...
                          className="p-1.5 text-green-600 hover:bg-green-100 rounded-md transition-all"
                          title="Accepter"
                        >
                          <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={2} stroke="currentColor" className="w-4 h-4">
                    ...
                    <button 
                      ...
                      className="p-1.5 text-[#31a7df] hover:bg-blue-50 rounded-md transition-colors"
                      title="Modifier"
                    >
                      <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={2} stroke="currentColor" className="w-4 h-4">
              ...
              {users.length === 0 && (
                <tr>
                  <td colSpan="5" className="py-12 text-center">
                    ...
                    <p className="text-gray-500 font-bold text-xs">Aucun membre de l'équipe</p>
                  </td>
                </tr>
              )}
            </tbody>
